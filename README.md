## shapes
[![Build Status](https://travis-ci.org/uwescience/shapes.svg?branch=master)](https://travis-ci.org/uwescience/shapes)
...
`shapes/shapes.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `shapes.py` in directory called
`shapes`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import shapes as sb` in an
interactive Python session, the classes and functions defined inside of the
`shapes.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the shapes module, the main classes defined at the bottom of
...
Either way, you can create a `shapes/data` folder in which you can
...
(typically, something like our `shapes/tests/test_shapes.py`). Within each
...
have one such test in `shapes/tests/test_shapes.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/shapes`),
and type:
...
will see a message such as:
    ...
    shapes/tests/test_shapes.py:49: AssertionError
...
To document `shapes` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/shapes/blob/master/doc/_static/RTD-advanced-conf.png)

 http://shapes.readthedocs.org/en/latest/
...
A `shapes/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/shapes) for the software. This
...
`shapes` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `shapes`, we use the
...
[here](https://github.com/uwescience/shapes/blob/master/scripts/Figure1.ipynb).
...
### Using `shapes` as a template

Let's assume that you want to create a small scientific Python project
called `shapes`. Maybe you already have some code that you are
...
button on the front page of the "shapes" repository.

In "Repository name" enter the name of your project. For example, enter 
`shapes` here. After that, you can hit the "Create repository from template" 
button.
...
`shapes/shapes.py` to be called `shapes/shapes.py`

	git mv shapes shapes
	git mv shapes/shapes.py shapes/shapes.py
	git mv shapes/tests/test_shapes.py shapes/tests/test_shapes.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `shapes` to `shapes`"

You will probably want to remove all the example data:

	git rm shapes/data/*
	git commit -a -m"Removed example `shapes` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `shapes` in them. Type
the following to see where all these files are:

	git grep shapes

You can replace `shapes` for `shapes` quickly with:

	git grep -l 'shapes' | xargs sed -i 's/shapes/shapes/g'
...
Edit all the mentions of `shapes` in the `shapes/__init__.py` file, and in
the `shapes/version.py` file as well.
